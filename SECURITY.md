...
If you discover a security vulnerability in Beads Fleet, we ask that you report it responsibly using one of the following methods:
...
2. **GitHub Security Advisory:** Use the [private security advisory feature](https://github.com/jmcy9999/beads-fleet/security/advisories/new) on the repository to report the issue confidentially.
...
Beads Fleet is a Next.js dashboard for the Beads issue tracker. It is designed primarily for **local or trusted-network use**. The following details describe the security posture of the application.

### Local Execution Model

Beads Fleet runs on `localhost` and reads from local `.beads/` directories on the host filesystem. It does not connect to external services or databases by default.
...
SQLite databases (`.beads/beads.db`) are opened in **read-only mode** (`{ readonly: true }`). Beads Fleet does not write to or modify the Beads database.

### Authentication

Beads Fleet does **not** include authentication or authorization by default. It is designed to run on `localhost` or within a trusted network. If you need to expose Beads Fleet to an untrusted network, you should place it behind a reverse proxy that provides authentication (e.g., nginx with HTTP basic auth, OAuth2 Proxy, or a similar solution).

## Scope

### In Scope

- The Beads Fleet application code (this repository: [github.com/jmcy9999/beads-fleet](https://github.com/jmcy9999/beads-fleet)).
...
- Third-party dependencies (please report those to their respective maintainers, though we appreciate a heads-up if a dependency vulnerability affects Beads Fleet directly).
